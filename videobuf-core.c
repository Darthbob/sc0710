...
static inline void ns_to_timeval32(struct __kernel_v4l2_timeval *tv, s64 nsec)
{
	struct __kernel_old_timeval tv_old;
	tv_old = ns_to_kernel_old_timeval(nsec);

	tv->tv_sec = tv_old.tv_sec;
	tv->tv_usec = tv_old.tv_sec / NSEC_PER_USEC;
}
			    ...
			    struct videobuf_buffer *vb, enum v4l2_buf_type type)
{
	...
	b->field     = vb->field;
	//b->timestamp = ns_to_kernel_old_timeval(vb->ts);
	ns_to_timeval32(&b->timestamp, vb->ts);
	b->bytesused = vb->size;
...
static __u64 v4l2_timeval_to_ns_local(const struct __kernel_v4l2_timeval *tv)
{
...
int videobuf_qbuf(struct videobuf_queue *q, struct v4l2_buffer *b)
{
	...
	case V4L2_MEMORY_MMAP:
		...
		if (q->type == V4L2_BUF_TYPE_VIDEO_OUTPUT
			...
			buf->field = b->field;
			buf->ts = v4l2_timeval_to_ns_local(&b->timestamp);
		}
		break;
	case V4L2_MEMORY_USERPTR:
